...
const URL_PATTERNS = {
  youtube: /(?:(?:music\.)?youtube\.com\/(?:watch\?(?:.*&)?v=|shorts\/|embed\/|live\/|clip\/|playlist\?list=)|youtu\.be\/|youtube\.com\/channel\/|youtube\.com\/@|youtube\.com\/c\/)/i,
...
function isValidUrlForPlatform(url, platform) {
  ...
  if (platform === 'youtube') {
    return URL_PATTERNS.youtube.test(trimmed) || /youtube\.com|youtu\.be|music\.youtube/i.test(trimmed);
  }
...
const PLATFORMS = (t) => [
  ...
  {
    id: 'lyrics', cls: 'ly', label: t('tab_lyrics'),
    icon: 'fa-solid fa-music',
    color: '#8b5cf6', glow: 'rgba(139,92,246,0.3)',
    placeholder: t('lyrics_search_placeholder'),
    desc: t('lyrics_desc'),
    features: [t('lyrics_feat1'), t('lyrics_feat2')],
    isLyrics: true,
  },
];

const PIB_CLS = { youtube: 'pib-yt', tiktok: 'pib-tt', instagram: 'pib-ig', pinterest: 'pib-pi', facebook: 'pib-fb', lyrics: 'pib-ly' };
...
function App() {
  ...
  useEffect(() => {
    if (!visitTracked.current) {
      ...
      trackStat('visit');
      detectCountryAndSetLanguage(i18n);
    }
  }, [i18n]);
  ...
  const handlePaste = async () => {
    try {
      ...
      const cleaned = pid === 'instagram' ? cleanInstagramUrl(text) : (pid === 'youtube' ? cleanYouTubeUrl(text) : text);
      setUrl(cleaned);
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (pid === 'youtube') {
        const cleanYtUrl = cleanYouTubeUrl(url);

        const [infoRes, oeRes] = await Promise.allSettled([
          fetch('/.netlify/functions/media-fetch', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'yt-info', url: cleanYtUrl }),
          }).then(r => r.json()),
          fetch(`https://www.youtube.com/oembed?url=${encodeURIComponent(cleanYtUrl)}&format=json`).then(r => r.json()),
        ]);

        const infoData = infoRes.status === 'fulfilled' ? infoRes.value : null;
        const oeData = oeRes.status === 'fulfilled' ? oeRes.value : null;

        const ytId = cleanYtUrl.match(/v=([a-zA-Z0-9_-]{11})/)?.[1];
        const title = oeData?.title || infoData?.data?.title || '';
        const image = oeData?.thumbnail_url || infoData?.data?.thumbnailHD || infoData?.data?.thumbnail || (ytId ? `https://img.youtube.com/vi/${ytId}/hqdefault.jpg` : null);
        const author = oeData?.author_name || infoData?.data?.author || '';
        ...
        if (data?.status?.success && data.data) {
          ...
          if (imagesList.length > 0) {
            pickerItems = imagesList.map((img, idx) => ({ url: typeof img === 'string' ? img : (img.url || img.thumb || img), thumb: typeof img === 'string' ? img : (img.thumb || img.url || img), id: idx }));
          } else if (mediaItems.length > 0) {
            pickerItems = mediaItems.map((m, idx) => ({
        ...
        const res = await fetch('/.netlify/functions/media-fetch', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'pinterest', url: url.trim() }),
        });
        const d = await res.json();
        if (!d.status?.success || !d.data) {
          throw new Error(d.error || t('error_fetching'));
        }
        const pin = d.data;
        const isGallery = pin.type === 'gallery' && pin.images?.length > 1;
        let pickerItems = [];
        if (isGallery) {
          pickerItems = pin.images.map((img, idx) => ({ url: img, thumb: img, type: 'image', id: idx }));
        }
      ...
      <main>
        ...
        <AnimatePresence mode="popLayout" initial={false}>
          ...
          >
            ...
            {isLyrics ? (
              <LyricsView onTrackStat={trackStat} />
            ) : isYouTube ? (
            ...
            {!isLyrics && !result && !loading && !searchLoading && searchResults.length === 0 && (
